    ...
    def trip_val(self, request):
        ...
        if start_date:
            ...
            if start_date < date.today():
                errors.append('Start date must be today or later.')
        else:
        ...
        if end_date:
            ...
            if end_date < date.today():
                errors.append('Return date must be today or later.')
        else:
            errors.append('Please add an end date.')

        if start_date and end_date:
            if start_date > end_date:
                errors.append('Return date must be after or the same as start date.')

        if not request.POST['destination']:
            errors.append('Enter a destination.')
        if not request.POST['description']:
            errors.append('Enter add a description.')
